...
def send_msg(sock, msg ,id):
    ...
    msg = struct.pack('>I',int(id)) + struct.pack('>I', len(msg)) + msg #将里程计测得的小车实时速度信息发送
    sock.sendall(msg)

def robot_cmd_Callback(msg): #接收速度信息，并按照socket协议模式，将所有速度信息以字符串的形式累加，配合其各自的id进行发送
...
rospy.init_node('robot3_cmd_vel_node')
 
r = rospy.Rate(0.2)
while not rospy.is_shutdown():
    rospy.Subscriber('/robot_3/cmd_vel',Twist,robot_cmd_Callback)
    r.sleep()
    r.sleep()
