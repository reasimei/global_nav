...
def msg_construct(data):
    ...
    pose.pose.position.z = float(list[2])
    pose.pose.orientation.x = float(list[3])
    pose.pose.orientation.y = float(list[4])
    pose.pose.orientation.z = float(list[5])
    pose.pose.orientation.w = float(list[6])
...
rospy.init_node("move_goal_client_node")
move_goal_pub = rospy.Publisher("move_base_simple/goal",PoseStamped,queue_size=100)
r = rospy.Rate(1000)
