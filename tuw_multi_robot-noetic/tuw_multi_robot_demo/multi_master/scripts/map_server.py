...
def msg_construct(data):
    ...
    map_pub.header.frame_id = maptopic_name
...
map_com = rospy.get_param('map_com')
maptopic_name= rospy.get_param('maptopic_name')
mapupdatetopic_name = rospy.get_param('mapupdatetopic_name')
map_socket.bind(('',map_com))
map_socket.listen(10)
 
(client,address) = map_socket.accept()
 
rospy.init_node("map_client_node")
map_pub = rospy.Publisher(maptopic_name,OccupancyGrid,queue_size=100)
map_up_pub = rospy.Publisher(mapupdatetopic_name,OccupancyGridUpdate,queue_size=1000)
r = rospy.Rate(1000)
